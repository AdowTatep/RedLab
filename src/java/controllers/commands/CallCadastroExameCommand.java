 ...
 * @author Diego
 */
public class CallCadastroExameCommand implements CommandApp {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            // Pega o login da pessoa passado pela página
            String loginUser = request.getParameter("pessoa");
            
            // Recupera a pessoa do banco.
            Pessoa p = new PessoaJpaController(emf).findPessoaFromUsuarioByLogin(loginUser);
            
            //Preenche o Exame
            Exame exame = new Exame(request, p);
            
            //Testa se os campos foram preenchidos corretamente
            if(true){//!validaCamposCadastro(request, mensagens)) {
               //Tenta inserir no banco
               new ExameJpaController(emf).create(exame);

               //Retorna para a página do administrador.              
            ...
            Logger.getLogger(controllers.commands.CallCadastroExameCommand.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            //No final de tudo
